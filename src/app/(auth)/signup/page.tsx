...
import { Eye, EyeOff, Loader2, ArrowRight, DollarSign, CheckCircle } from 'lucide-react'
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-950 via-purple-950 to-slate-900 p-4">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-6">
          {['Track debts', 'Smart reminders', 'AI insights'].map((f) => (
            <span key={f} className="flex items-center gap-1 text-xs text-slate-400 bg-white/5 border border-white/10 rounded-full px-3 py-1">
              <CheckCircle className="w-3 h-3 text-purple-400" /> {f}
            </span>
          ))}
        </div>
        ...
        <Card className="border border-white/10 bg-white/5 backdrop-blur-xl shadow-2xl">
          <CardHeader className="space-y-1 pb-4">
            ...
            <CardDescription className="text-slate-400 text-center">
              Start tracking debts & credits for free
            </CardDescription>
          </CardHeader>
                      ...
                      {[1, 2, 3].map((level) => (
                        <div
                          ...
                          className={`h-1 flex-1 rounded-full transition-all duration-300 ${
                            passwordStrength >= level ? strengthColor[passwordStrength] : 'bg-white/10'
                          }`}
                        />
                      ))}
                    </div>
